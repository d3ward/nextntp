export class newsLoader {
    ...
    setFilter(f) {
        ...
        console.log(
            '🚀 ~ file: newsLoader.js ~ line 20 ~ newsLoader ~ setFilter ~ this.filters',
            this.filters
        )
    }
        ...
        var json = await fetch(url, { method: 'GET', credentials: 'include' })
            ...
            .then((html) => {
                ...
                articles.forEach(function (node) {
                    ...
                    const link =
                        item
                            .querySelector('a[href^="./article"]')
                            .href.replace('./', 'https://news.google.com/') ||
                        false
                    ...
                    const mainArticle = {
                        ...
                        source: item.querySelector('div.wsLqz > a')
                            ? item.querySelector('div.wsLqz > a').innerText
                            : false,
                        ...
                        time: item.querySelector('time')
                            ...
                            : false,
                        datetime: item.querySelector('time')
                            ? item
                                  .querySelector('time')
                    ...
                    if (mainArticle.title && mainArticle.source) {
                        ...
                        if (!filter_result) result.items.push(mainArticle)
                    }
                })
            ...
            .catch((error) => {
                console.log(
                    '🚀 ~ file: newsLoader.js ~ line 80 ~ newsLoader ~ .then ~ error',
                    error
                )
                return {
                    topic: topic_,
