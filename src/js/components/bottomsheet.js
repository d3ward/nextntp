export class BottomSheet {
    ...
    init() {
        this.bottomSheet
            ...
            .addEventListener(
                this.options.eventMap.start,
                this.startDrag.bind(this),
                { passive: false }
            )
        this.bottomSheet
            .querySelector('.bs-scrim')
            .addEventListener('click', this.hide.bind(this), { passive: false })
        this.bottomSheet
            .querySelector('.bs-footer')
            .addEventListener('click', this.hide.bind(this), { passive: false })
    }
    ...
    startDrag(event) {
        ...
        const onMove = (moveEvent) => {
            ...
            if (offsetY < 40) {
                ...
                if (offsetY * -1 >= this.options.threshold)
                    navigator.vibrate(100)
                this.bottomSheet.querySelector('.bs-sheet').style.transform =
                    `translateY(${m}px)`
            }
        }
        ...
        document.addEventListener(this.options.eventMap.move, onMove, {
            passive: false
        })
        document.addEventListener(this.options.eventMap.end, onEnd, {
            passive: false
        })
    }
