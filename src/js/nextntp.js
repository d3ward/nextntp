...
function theme() {
  ...
  if (window.CSS && CSS.supports('color', 'var(--bg)') && toggles) {
    ...
    if (storedTheme === 'light') metaTColor.setAttribute('content', mtc.light)
...
const isTD = is_td()
var mLstnr = (!isTD)?['mousemove', 'mouseup', 'mousedown']:['touchmove', 'touchend', 'touchstart']
...
if (is_incognito()) {
    ...
    Object.keys(ntp_sb.se).forEach((el, index) => {
      ...
      if (se_item != undefined && se_status) {
        ...
        si.addEventListener('click', (event) => {
          ...
          if (sei == undefined) sei = ntp_sb.custom_se[event.target.innerText]
  ...
  sb_custom_form.addEventListener('submit', (e) => {
    ...
    if (
      ...
      ntp_sb.se[name] = true
      ntp_sb.custom_se[name] = {'color':color, 'svg':svg, 'query':query}
      ls_set('ntp_sb', ntp_sb)
      f_setup_sb()
      render_se_list()
  ...
  if (ntp_sett.status[0]) {
    ...
    function search(text) {
      ...
      if (query == undefined)
        query = ntp_sb.custom_se[key].query
  ...
  if (ntp_sett.status[1]) {
    ...
    function f_dlg(view) {
		dlg_tg.style.display = (view==0 || view==4)?'inline':'none'
        dlg_fl.style.display = (view==5)?'inline':'none'
        b_add.style.display = (view==0 )?'inline':'none'
        b_add2.style.display = (view==2 )?'inline':'none'
        b_save.style.display = (view==4 || view==5 )?'inline':'none'
    ...
    function f_cnt() {
		if(t_url.value.trim().length>0 && get_root_domain(t_url.value).length>0){
      var newt = {}
    ...
    function f_sptt() {
      ...
      if (i_url.disabled) {
        ...
        timeoutVariable = setTimeout(function () {
          var iUrl =
            'https://logos.kiwibrowser.com/' + get_root_domain(t_url.value)
          p_tile.src = iUrl
          i_url.value = iUrl
        }, 1000)
      }
  ...
  if (ntp_sett.status[2]) {
    ...
    function add_gnews(title, news_time, source, source_logo, link, image) {
      ...
      if (localStorage.itemsNews && localStorage.itemsNews.indexOf(itemID) > -1)
        return
      var itemsNews = []
      ...
      if (image != null)
        imgrender =
          '<img src="' + image + '" class="news_img" alt="Article Source" />'
      innerDiv.innerHTML =
        imgrender +
        '<div class="news_cnt">' +
    ...
    function render_gnews(answer) {
      ...
      for (var i = articles.length - 1; i > 0; i--) {
        ...
        if (link)
          link = link.replace(/.+?(?=articles)/, 'https://news.google.com/')
        if (link && image && title) {
  ...
  function f_wallp4() {
    ...
    ntp_bdy.style.setProperty('--bg-c' + (ntp_theme == 'dark' ? 'd' : 'l'), v)
    save_ntpbdy()
  ...
  var picker = new Picker({
    ...
    onDone: function (color) {
      console.log('cp_type : ', cp_type, ' cp number: ', cp_current_el)
      if (cp_type == 'color_cl' || cp_type == 'color_cld') {
        ...
        document.getElementById('sett_' + cp_type).style.background = color.hex
        mtc[cp_type == 'mtcl' ? 'light' : 'dark'] = color.hex
        document
  ...
  document
    .querySelector('#cp_v  .picker_cancel>button')
  ...
  function f_cp_rgb(type, number) {
    ...
    console.log('cp_type : ' + cp_type, ' color : ' + color)
    picker.setColor(color, true)
    cl_vn.show()
  }
